...
import { type ProfileData, type UpdateProfileRequest } from "@/services/profileService";
                    ...
                    <div className="flex-1 space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-center">
                            ...
                            <div className="cursor-pointer" onClick={openFollowers}>
                                <div className="text-2xl font-bold">{followers || 0}</div>
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <Input id="firstName" value={editForm.firstName} onChange={(e) => setEditForm(prev => ({ ...prev, firstName: e.target.value }))} className="border-gray-200" />
                        </div>
                        <div>
                            <Label htmlFor="lastName">Last Name</Label>
                            <Input id="lastName" value={editForm.lastName} onChange={(e) => setEditForm(prev => ({ ...prev, lastName: e.target.value }))} className="border-gray-200" />
                        </div>
                    </div>
                    <div>
                        <Label htmlFor="username">Username</Label>
                        <Input id="username" value={editForm.username} onChange={(e) => setEditForm(prev => ({ ...prev, username: e.target.value }))} className="border-gray-200" />
                    </div>

                    {/* Bio */}
                    <div>
                        <Label htmlFor="bio">Bio</Label>
                        <Textarea id="bio" rows={3} value={editForm.bio} onChange={(e) => setEditForm(prev => ({ ...prev, bio: e.target.value }))} className="border-gray-200" />
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {categories?.map(category => {
                                ...
                                console.log(`Category ${category.name} (ID: ${category.id}) selected:`, selected);
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded-full text-sm ${selected ? 'bg-primary text-white' : 'bg-gray-200 text-gray-800'
                                            }`}
                                    >
                                        {category.name}
                                    </button>
