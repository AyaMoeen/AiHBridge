...
interface FollowersResponse {
    count: number;
    results: Array<{ id: number; follower: BackendProfile; created_at: string }>;
}
interface FollowingResponse {
    count: number;
    results: Array<{ id: number; following: BackendProfile; created_at: string }>;
}
...
// --- Frontend Interfaces ---
export interface InterestedCategory {
    id?: number;
    text?: string;
}
...
class ProfileService {
    ...
    async getFollowStatus(userId: number): Promise<FollowStatus> {
        try {
            ...
            const isFollowing = following.some((f) => f.following === userId);
            const isFollowedBy = followers.some((f) => f.follower === currentUserId);
    ...
    async uploadAvatar(file: File): Promise<string> {
        try {
            if (file.size > 5 * 1024 * 1024)
                throw new ProfileServiceError("File size must be less than 5MB");
            const allowedTypes = [
                "image/jpeg",
                "image/png",
                "image/gif",
            ...
            if (!allowedTypes.includes(file.type))
                throw new ProfileServiceError("Invalid image format");

            const formData = new FormData();
            formData.append("profile_picture", file);

            const res = await api.patch<BackendProfile>(
    ...
    async updateProfile(
        updates: UpdateProfileRequest & { interested_category_ids?: number[] }
    ): Promise<User> {
        try {
            const payload: any = {};
            if (updates.firstName || updates.lastName)
                payload.name = `${updates.firstName || ""} ${updates.lastName || ""
                    }`.trim();
    ...
    //       const res = await api.patch<BackendProfile>("/profiles/", formData, {
    //         headers: { "Content-Type": "multipart/form-data" },
    //       });
    //       this.clearCache();
    //       return this.mapBackendToFrontend(res.data).avatar || "";
    ...
    //       const isFollowing = following.results.some(
    //         (f) => f.following.id === userId
    //       );
    //       const isFollowedBy = followers.results.some(
    //         (f) => f.follower.id === currentUserId
    //       );
    ...
    async searchUsers(query: string, page = 1, limit = 10): Promise<User[]> {
        try {
            ...
            return res.data.results.map((profile: BackendProfile) =>
                this.mapBackendToFrontend(profile)
            );
        } catch (error: any) {
